...
class PredictiveNeuralNet:
    ...
    def load(file_name):
        ...
        loaded_neural_net = pickle.load(binary_file)
        binary_file.close()
        return loaded_neural_net

# Example usage:
#
# # create a net of 4 layers with two features, 2 hidden layers of 5 and 10 nodes respectively, and 2 outputs
# nn = PredictiveNeuralNet([2,10,5,2])
#
# # get the computed actions based on the feature input values
# nn.setInputData(np.array([1.2,0.4]))
#
# print(nn.predict())
# aa = nn.copy()
# #mutate the weights by 10%
# aa = aa.mutate(0.1)
